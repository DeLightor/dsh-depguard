...
export function checkDuplicate(copies, pkg) {
  ...
  return [{
    ...
    fix: '在 profile 目录运行 `pnpm why <pkg>` 定位来源，然后 `pnpm dedupe`；或在 pnpm-workspace.yaml 加 `overrides: { \'<pkg>\': \'<runtime 版本>\' }` 锁死单副本（pnpm≥10 已不再读 package.json 的 pnpm 字段；pnpm<10 则写 package.json 的 "pnpm": {"overrides": {...}}）',
...
export function checkDrift(copies, pkg) {
  ...
  for (const c of copies) {
    ...
    findings.push({
      ...
      fix: `在 pnpm-workspace.yaml 加 "overrides: { '${pkg}': '${base.version}' }" 锁死版本（pnpm≥10 已不再读 package.json 的 pnpm 字段；pnpm<10 则写 package.json 的 "pnpm": {"overrides": {...}}）`,
