  ...
  for (const pkg of SERVICE_PACKAGES) {
    ...
    if (!rangeMatches(range, base)) {
      prediction.push({
        ...
        fix: `安装后在 pnpm-workspace.yaml 加 "overrides: { '${pkg}': '${base}' }"（pnpm≥10 已不再读 package.json 的 pnpm 字段；pnpm<10 则写 package.json 的 "pnpm": {"overrides": {...}}）锁死`,
      })
    }
  }
...
export function rangeMatches(range, version) {
  ...
  // 精确版本字符串（含 pre-release 后缀）：直接比较，避免 Number('0-rc.6')=NaN
  if (/^\d+\.\d+\.\d+(-\S+)?$/.test(v) && /^[\^~]?\d+\.\d+\.\d+(-\S+)?$/.test(r)) {
    const stripped = r.replace(/^[\^~]/, '')
    if (v === stripped) return true
  }
  if (!/^[\^~]?\d+\.\d+\.\d+/.test(r)) return true // 复合/特殊范围：保守放行
  // version 带 pre-release 而 range 不带：严格 semver 中 pre-release 低于正式版，不匹配
  if (v.includes('-') && !r.includes('-')) return false
  const m = r.match(/^([\^~]?)(\d+)\.(\d+)\.(\d+)/)
  // 只取数字部分，忽略 pre-release 后缀（Number('0-rc.6') 会是 NaN）
  const target = v.split(/[-.]/).slice(0, 3).map(Number)
  const base = [Number(m[2]), Number(m[3]), Number(m[4])]
  const op = m[1]
  if (op === '^') {
    // caret 语义：^0.x.y 中 minor 是 breaking（0.x 只允许 patch 升级）
    if (base[0] === 0) {
      return target[0] === 0 && target[1] === base[1] && target[2] >= base[2]
    }
...
export async function predict({ target, root } = {}) {
  ...
  if (target.startsWith('github:')) {
    try {
      // 注意：slice('github:') 会把字符串参数转 NaN → slice(0) → 前缀剥不掉！
      // 必须用 replace 剥前缀。
      manifest = await fetchGithubManifest(target.replace(/^github:/, ''))
    } catch (e) {
      fetchError = e.message
    }
  } else if (target.startsWith('./') || target.startsWith('/') || target.startsWith('file:') || /^[A-Za-z]:[\\/]/.test(target)) {
    // 本地目录：含 Windows 盘符绝对路径（F:/... 或 F:\...）——此前盘符路径
    // 会被误判为 npm 包名走 registry 查询导致预测失败。
